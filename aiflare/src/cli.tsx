...
import type { AuthDotJson } from "./backend/authModel.js";
import { loadAuthDotJsonSync } from "./backend/authModel.js";
import { createInputItem } from "./utils/input-utils";
import { initLogger } from "./utils/logger/log";
import { installFetchLogger } from "./utils/network-logger.js";
...
const cli = meow(
  ...
  Options
    ...
    -m, --model <model>             Model to use for completions (default: gpt-5.1-codex)
    ...
    --reasoning <effort>      Set the reasoning effort level (low, medium, high) (default: medium)
...
// Handle 'logout' subcommand similar to Rust CLI:
// - Delete ~/.codey/auth.json if present
// - Print a human-friendly status message
// - Exit without starting the main app
...
let existingAuth: AuthDotJson | null = null;
try {
  existingAuth = loadAuthDotJsonSync();
  if (existingAuth?.tokens) {
    savedTokens = {
      id_token: existingAuth.tokens.idToken.rawJwt,
      access_token: existingAuth.tokens.accessToken,
      refresh_token: existingAuth.tokens.refreshToken,
    };
  }
  const lastRefreshTime = existingAuth?.last_refresh
    ? new Date(existingAuth.last_refresh).getTime()
    : 0;
  const expired = Date.now() - lastRefreshTime > 28 * 24 * 60 * 60 * 1000;
  if (existingAuth?.OPENAI_API_KEY && !expired) {
    apiKey = existingAuth.OPENAI_API_KEY;
  }
} catch {
  // ignore errors
}
...
if (provider.toLowerCase() === "openai" && (!apiKey || cli.flags.login)) {
  const forceLogin = cli.flags.login || !existingAuth?.tokens;
  apiKey = await fetchApiKey(client.issuer, client.client_id, forceLogin);

  // Refresh savedTokens after a successful login so that follow-up flows
  // (e.g. --free credit redemption) have access to the latest refresh token.
  try {
    existingAuth = loadAuthDotJsonSync();
    if (existingAuth?.tokens) {
      savedTokens = {
...
// Debug logging: record the effective model/provider and auth context so that
// we can compare CLI runs with integration tests. Only written when DEBUG is
// set, via the async logger.
try {
  // eslint-disable-next-line @typescript-eslint/no-var-requires
  const { log } = require("./utils/logger/log.js") as {
    log: (msg: string) => void;
  };
  const maskedKey =
    apiKey && apiKey.length > 10
      ...
      : apiKey || "<none>";
  const baseUrl = getBaseUrl(provider) ?? "<undefined>";
  const project =
    process.env["OPENAI_PROJECT"] ||
    (() => {
      try {
        // Lazy import to avoid cycles; fall back silently on error.
        // eslint-disable-next-line @typescript-eslint/no-var-requires
        const { getOpenaiProject } = require("./utils/config.js") as {
          getOpenaiProject: () => string | undefined;
        };
        return getOpenaiProject() ?? "";
      } catch {
        return "";
      }
    })();
  ...
  log(
    `[startup] model=${config.model} provider=${provider} baseUrl=${baseUrl} apiKey=${maskedKey} org=${process.env["OPENAI_ORGANIZATION"] || ""} project=${project || "<none>"}`,
  );
} catch {
  // ignore logging errors – CLI should still start
}
