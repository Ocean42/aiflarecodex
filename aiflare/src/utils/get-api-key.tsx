...
import { loadAuthDotJson, parseIdToken, saveAuthDotJson } from "../backend/authModel.js";
...
async function maybeRedeemCredits(
  ...
  try {
    ...
    if (!idClaims || !idClaims.exp || Date.now() >= idClaims.exp * 1000) {
      ...
      try {
        ...
        if (refreshData.refresh_token) {
          try {
            const existing = await loadAuthDotJson();
            if (existing?.tokens) {
              const idTokenInfo = parseIdToken(currentIdToken);
              const updated: AuthDotJson = {
                OPENAI_API_KEY: existing.OPENAI_API_KEY,
                tokens: {
                  idToken: idTokenInfo,
                  accessToken: existing.tokens.accessToken,
                  refreshToken:
                    refreshData.refresh_token ?? existing.tokens.refreshToken,
                  accountId:
                    existing.tokens.accountId ?? idTokenInfo.chatgptAccountId,
                },
                last_refresh: new Date().toISOString(),
              };
              await saveAuthDotJson(updated);
            }
          } catch (err) {
            // eslint-disable-next-line no-console
    ...
    if (!redeemRes.ok) {
      try {
        // Best-effort parse to aid debugging, but don't fail the CLI.
        // The backend endpoint may change independently of this legacy
        // client, so treat 4xx as "no credits available" instead of a
        // hard error.
        // eslint-disable-next-line @typescript-eslint/no-unsafe-assignment
        const body = await redeemRes.json();
        // eslint-disable-next-line no-console, @typescript-eslint/no-unsafe-assignment
        console.warn(
          redeemRes.status === 400
            ? "Credit redemption is not available via this flow anymore or no credits are eligible to be redeemed. Continuing without applying credits."
            : `Credit redemption request failed: ${redeemRes.status} ${redeemRes.statusText} – ${JSON.stringify(
                body,
              )}`,
        );
      } catch {
