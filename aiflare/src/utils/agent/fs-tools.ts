...
type NormalizedIndentationOptions = {
  anchorLine: number;
  maxLevels: number;
...
type NormalizedReadFileArgs = {
  filePath: string;
  offset: number;
  limit: number;
  mode: ReadMode;
  indentation?: NormalizedIndentationOptions;
};
...
export async function runListDirTool(
  rawArgs: Record<string, unknown>,
): Promise<{ output: string; absolutePath: string }> {
  ...
  const selected = entries.slice(startIndex, endIndex).sort((a, b) =>
...
export async function runGrepFilesTool(
  rawArgs: Record<string, unknown>,
): Promise<{
  output: string;
  searchPath: string;
...
async function normalizeReadFileArgs(
  rawArgs: Record<string, unknown>,
): Promise<NormalizedReadFileArgs> {
  ...
  const rawMode =
    typeof rawArgs.mode === "string" ? rawArgs.mode.toLowerCase() : "slice";
  const mode: ReadMode =
    rawMode === "indentation" ? "indentation" : "slice";

  let indentation: NormalizedIndentationOptions | undefined;
  if (mode === "indentation") {
    const rawIndent =
      typeof rawArgs.indentation === "object" && rawArgs.indentation
        ? (rawArgs.indentation as Record<string, unknown>)
        : {};
    const anchorLine = parsePositiveInt(
      rawIndent.anchor_line,
      offset,
...
async function normalizeGrepArgs(
  rawArgs: Record<string, unknown>,
): Promise<NormalizedGrepArgs> {
  const pattern =
    typeof rawArgs.pattern === "string" ? rawArgs.pattern.trim() : "";
  if (!pattern) {
    throw new Error("pattern must not be empty");
  }
  const include =
    typeof rawArgs.include === "string" && rawArgs.include.trim() !== ""
...
async function normalizeListDirArgs(
  rawArgs: Record<string, unknown>,
): Promise<ListDirArgs> {
...
async function resolveFilePath(requestedPath: string): Promise<string> {
...
function resolveDirectoryPath(requestedPath: string): string {
  if (path.isAbsolute(requestedPath)) {
    return requestedPath;
  }
...
async function assertIsFile(filePath: string): Promise<void> {
  try {
...
async function assertIsDirectory(dirPath: string): Promise<void> {
  try {
...
async function collectFileLines(filePath: string): Promise<Array<LineRecord>> {
  let data: string;
  try {
  ...
  const records: Array<LineRecord> = [];
...
function readSlice(
  records: Array<LineRecord>,
  args: NormalizedReadFileArgs,
): Array<string> {
...
function readIndentationBlock(
  ...
  const deque: Array<LineRecord> = [records[anchorIndex]];
  let i = anchorIndex - 1;
  let j = anchorIndex + 1;
  let iCounter = 0;
  let jCounter = 0;
...
function formatRecordLine(record: LineRecord): string {
  return `L${record.number}: ${record.display}`;
}

function parseRgOutput(stdout: string, limit: number): Array<string> {
...
function computeEffectiveIndents(records: Array<LineRecord>): Array<number> {
  const effective: Array<number> = [];
  let previous = 0;
...
function trimEmptyEdges(records: Array<LineRecord>): void {
...
async function collectEntries(
  dirPath: string,
  depth: number,
): Promise<Array<DirEntryRecord>> {
  const entries: Array<DirEntryRecord> = [];
  ...
  while (queue.length > 0) {
    ...
    for (const dirent of dirEntries) {
      ...
      const displayDepth =
        rel === "" ? 0 : rel.split(path.sep).filter(Boolean).length;
...
function classifyDirent(dirent: Dirent): DirEntryKind {
...
function formatEntryLine(entry: DirEntryRecord): string {
