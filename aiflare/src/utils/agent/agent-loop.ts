...
const DEFAULT_INCLUDE_FIELDS: ResponseCreateParams["include"] = [
...
const listMcpResourcesTool: FunctionTool = {
  type: "function",
...
const listMcpResourceTemplatesTool: FunctionTool = {
  type: "function",
...
const readMcpResourceTool: FunctionTool = {
  type: "function",
...
const applyPatchFunctionTool: FunctionTool = {
  type: "function",
  name: "apply_patch",
  description:
...
const viewImageTool: FunctionTool = {
  type: "function",
  name: "view_image",
  description:
...
export class AgentLoop {
  ...
  constructor({
    ...
    log(
      `[agent-init${labelSuffix}] model=${this.model} provider=${this.provider} baseUrl=${this.baseUrl ?? "<undefined>"} apiKey=${this.apiKeyMasked} org=${this.orgHeader ?? "<none>"} project=${this.projectHeader ?? "<none>"}`,
    );
      ...
      while (turnInput.length > 0) {
        ...
        for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
          try {
            ...
            let effectiveParams: ResponseCreateParams = params;
            if (isCodexBackend) {
              ...
              const minimal: ResponseCreateParams = {
                model: params.model,
                ...
                tool_choice: "auto",
                parallel_tool_calls:
                  params.parallel_tool_calls ?? turnConfig.parallelToolCalls,
              ...
              const promptCacheKey = (params as {
                prompt_cache_key?: string;
              }).prompt_cache_key;
              if (promptCacheKey) {
                (
                  minimal as ResponseCreateParams & {
                    prompt_cache_key?: string;
                  }
            ...
            const prevId =
              (params as { previous_response_id?: string })
                .previous_response_id ?? "";
            const requestUrl =
              (this.baseUrl ?? "<undefined>").replace(/\/+$/, "") +
              "/responses";
            logHttpDebug({
              phase: "request",
              method: "POST",
              ...
              extra: {
                store: String((effectiveParams as { store?: boolean }).store),
                prevId,
                tools: toolNames,
              },
            });
            ...
            const errorBody =
              typeof errSnapshot.body === "string"
                ? errSnapshot.body
                : errSnapshot.body
            ...
            if (isClientError) {
              ...
              try {
                ...
                log(
                  `[agent-error] client_error status=${status ?? "unknown"} payload=${JSON.stringify(
                    safeParams,
                  )}`,
                );
              } catch {
