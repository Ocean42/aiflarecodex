...
esbuild
  ...
  .then(async () => {
    const resourceSrc = path.resolve("src/resources/codex-instructions.txt");
    const resourceDestDir = path.resolve(OUT_DIR, "resources");
    const resourceDest = path.join(resourceDestDir, "codex-instructions.txt");
    await fs.promises.mkdir(resourceDestDir, { recursive: true });
    await fs.promises.copyFile(resourceSrc, resourceDest);
  })
