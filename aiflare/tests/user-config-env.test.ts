...
/**
 * Verifies that ~/.codey.env is parsed (lowest-priority) when present.
 */

describe("user‑wide ~/.codey.env support", () => {
  ...
  beforeEach(() => {
    ...
    // Write ~/.codey.env with a dummy key.
    writeFileSync(
      join(tempHome, ".codey.env"),
  ...
  it("loads the API key from ~/.codey.env when not set elsewhere", async () => {
