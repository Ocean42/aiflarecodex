...
- **macOS 12+** - commands are wrapped with **Apple Seatbelt** (`sandbox-exec`).
  ...
  - Everything is placed in a read-only jail except for a small set of
    writable roots (`$PWD`, `$TMPDIR`, `~/.codey`, etc.).
...
1. `~/.codey/AGENTS.md` - personal global guidance
...
Codey configuration files can be placed in the `~/.codey/` directory, supporting both YAML and JSON formats.
...
1. YAML format (save as `~/.codey/config.yaml`):
...
2. JSON format (save as `~/.codey/config.json`):
...
You can create a `~/.codey/AGENTS.md` file to define custom guidance for the agent:
