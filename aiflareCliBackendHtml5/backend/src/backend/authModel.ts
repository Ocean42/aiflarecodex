...
function normaliseTokenData(rawTokens) {
    if (!rawTokens || typeof rawTokens !== "object") {
        return undefined;
    }
    const legacy = rawTokens;
    const current = rawTokens;
    if (current.idToken &&
        ...
        return {
            ...
            accountId: typeof current.accountId === "string" &&
                current.accountId.trim() !== ""
                ? current.accountId
                : undefined,
        };
    }
    if (typeof legacy.id_token === "string" &&
        ...
        return {
            ...
            accountId: typeof legacy.account_id === "string" &&
...
export async function loadAuthDotJson() {
    ...
    try {
        ...
        const auth = {
            OPENAI_API_KEY: typeof raw.OPENAI_API_KEY === "string"
                ? raw.OPENAI_API_KEY
                : undefined,
            tokens,
            last_refresh: typeof raw.last_refresh === "string" ? raw.last_refresh : undefined,
        };
        return auth;
    }
    catch (err) {
...
export function loadAuthDotJsonSync() {
    const authFile = getAuthFilePath();
...
export function getAuthDebugInfoSync() {
    const auth = loadAuthDotJsonSync();
    if (!auth) {
        return null;
    }
    const hasApiKey = typeof auth.OPENAI_API_KEY === "string" &&
...
export function getProjectIdFromAuthSync() {
    const auth = loadAuthDotJsonSync();
    if (!auth?.tokens) {
        return null;
    }
...
export async function saveAuthDotJson(auth) {
    const authFile = getAuthFilePath();
