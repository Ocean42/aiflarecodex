...
// required.
export function parseToolCallOutput(toolCallOutput) {
    try {
        const parsed = JSON.parse(toolCallOutput);
        const output = typeof parsed.output === "string"
            ? parsed.output
            : JSON.stringify(parsed.output ?? "");
        const metadata = parsed.metadata && typeof parsed.metadata.exit_code === "number"
            ? parsed.metadata
            : {
                exit_code: 0,
                duration_seconds: 0,
 ...
 */
export function parseToolCallArguments(toolCallArguments) {
    let json;
    try {
    ...
    // Accept both shapes:
    const commandArray = toStringArray(cmd) ??
...
function toStringArray(obj) {
    if (Array.isArray(obj) && obj.every((item) => typeof item === "string")) {
        const arrayOfStrings = obj;
