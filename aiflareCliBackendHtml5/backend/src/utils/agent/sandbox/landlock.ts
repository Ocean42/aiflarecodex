 ...
 */
export async function execWithLandlock(cmd, opts, userProvidedWritableRoots, config, abortSignal, onChunk) {
    const sandboxExecutable = await getSandboxExecutable();
    const extraSandboxPermissions = userProvidedWritableRoots.flatMap((root) => ["--sandbox-permission", `disk-write-folder=${root}`]);
    const fullCommand = [
        sandboxExecutable,
        "--sandbox-permission",
 ...
 */
let sandboxExecutablePromise = null;
async function detectSandboxExecutable() {
    ...
    while (true) {
        try {
            ...
            await fs.promises.access(path.join(dir, "package.json"), fs.constants.F_OK);
 ...
 * ```
 */
function verifySandboxExecutable(sandboxExecutable) {
    ...
    return new Promise((resolve, reject) => {
        ...
        execFile(sandboxExecutable, args, (error, stdout, stderr) => {
            if (error) {
                log(`Sandbox check failed for ${sandboxExecutable} ${args.join(" ")}: ${error}`);
                log(`stdout: ${stdout}`);
                log(`stderr: ${stderr}`);
...
function getLinuxSandboxExecutableForCurrentArchitecture() {
    switch (process.arch) {
        case "arm64":
