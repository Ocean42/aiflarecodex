...
export function execWithSeatbelt(cmd, opts, writableRoots, config, abortSignal, onChunk) {
    let scopedWritePolicy;
    let policyTemplateParams;
    ...
    log(`Running seatbelt with policy: ${fullPolicy} and ${policyTemplateParams.length} template params: ${policyTemplateParams.join(", ")}`);
    const fullCommand = [
