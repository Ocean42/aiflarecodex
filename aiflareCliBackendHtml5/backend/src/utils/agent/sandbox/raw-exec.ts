 ...
 */
export function exec(command, options, config, abortSignal, onChunk) {
    ...
    const child = spawn(prog, adaptedCommand.slice(1), fullOptions);
    ...
    return new Promise((resolve) => {
        ...
        const stdoutCollector = createTruncatingCollector(child.stdout, maxBytes, maxLines, (text) => emitChunk("stdout", text));
        const stderrCollector = createTruncatingCollector(child.stderr, maxBytes, maxLines, (text) => emitChunk("stderr", text));
        child.on("exit", (code, signal) => {
            ...
            resolve(addTruncationWarningsIfNecessary(execResult, stdoutCollector.hit, stderrCollector.hit));
        });
        child.on("error", (err) => {
            const execResult = {
                stdout: "",
 ...
 */
function addTruncationWarningsIfNecessary(execResult, hitMaxStdout, hitMaxStderr) {
