...
class Parser {
    ...
    parse_update_file(text) {
        ...
        while (!this.is_done([
            ...
            const [nextChunkContext, chunks, endPatchIndex, eof] = peek_next_section(this.lines, this.index);
            const [newIndex, fuzz] = find_context(fileLines, nextChunkContext, index, eof);
            if (newIndex === -1) {
...
function find_context(lines, context, start, eof) {
    if (eof) {
        let [newIndex, fuzz] = find_context_core(lines, context, lines.length - context.length);
        if (newIndex !== -1) {
...
function _get_updated_file(text, action, path) {
    ...
    for (const chunk of action.chunks) {
        if (chunk.orig_index > origLines.length) {
            throw new DiffError(`${path}: chunk.orig_index ${chunk.orig_index} > len(lines) ${origLines.length}`);
        }
...
if (import.meta.url === `file://${process.argv[1]}`) {
    ...
    process.stdin.on("end", () => {
        ...
        try {
            const result = process_patch(patchText, open_file, write_file, remove_file);
            // eslint-disable-next-line no-console
            console.log(result);
        }
        catch (err) {
