...
import { PATCH_SUFFIX } from "./parse-apply-patch.js";
 ...
 */
export function exec({ cmd, workdir, timeoutInMillis, additionalWritableRoots, onChunk, }, sandbox, config, abortSignal) {
    const opts = {
    ...
    switch (sandbox) {
        ...
        case SandboxType.MACOS_SEATBELT: {
            ...
            return execWithSeatbelt(cmd, opts, writableRoots, config, abortSignal, onChunk);
        }
        case SandboxType.LINUX_LANDLOCK: {
            return execWithLandlock(cmd, opts, additionalWritableRoots, config, abortSignal, onChunk);
        }
    }
}
export function execApplyPatch(patchText, workdir = undefined) {
    ...
    try {
        const result = process_patch(applyPatchInput, (p) => fs.readFileSync(resolvePathAgainstWorkdir(p, workdir), "utf8"), (p, c) => {
...
export function getBaseCmd(cmd) {
