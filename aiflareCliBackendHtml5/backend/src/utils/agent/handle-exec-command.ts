...
// ---------------------------------------------------------------------------
const alwaysApprovedCommands = new Set();
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
function deriveCommandKey(cmd) {
    // pull off only the bits you care about
    const [maybeShell, maybeFlag, coreInvocation,
    /* …ignore the rest… */
    ] = cmd;
...
export async function handleExecCommand(rawArgs, config, policy, additionalWritableRoots, getCommandConfirmation, abortSignal, hooks) {
    const args = rawArgs.workdir && rawArgs.workdir !== "/"
        ? rawArgs
    ...
    if (alwaysApprovedCommands.has(key)) {
        ...
        /* runInSandbox */ false, additionalWritableRoots, config, abortSignal).then(convertSummaryToResult);
    }
    ...
    switch (safety.type) {
        case "ask-user": {
            ...
            const review = await askUserPermission(args, safety.applyPatch, getCommandConfirmation, hooks, approvalContext);
            if (review != null) {
                return review;
            }
            runInSandbox = false;
    ...
    const runWithLifecycle = async (inSandbox) => {
        hooks?.onLifecycle?.({
            type: "begin",
            command,
            workdir: args.workdir,
        });
        const summary = await execCommand(args, applyPatch, inSandbox, additionalWritableRoots, config, abortSignal, hooks?.onChunk);
        hooks?.onLifecycle?.({
            type: "end",
            command,
    ...
    if (summary.exitCode !== 0 &&
        ...
        const review = await askUserPermission(args, safety.applyPatch, getCommandConfirmation, hooks, retryContext);
        if (review != null) {
            return review;
        }
        else {
...
function convertSummaryToResult(summary) {
...
async function execCommand(execInput, applyPatchCommand, runInSandbox, additionalWritableRoots, config, abortSignal, onChunk) {
    ...
    if (applyPatchCommand != null) {
        ...
        const timeout = timeoutInMillis != null
            ? Math.round(timeoutInMillis / 1000).toString()
            : "undefined";
        log(`EXEC running \`${formatCommandForDisplay(cmd)}\` in workdir=${resolvedWorkdir} with timeout=${timeout}s`);
    }
    ...
    const execResult = applyPatchCommand != null
        ...
        : await exec({
            ...
            onChunk,
        }, await getSandbox(runInSandbox), config, abortSignal);
    const duration = Date.now() - start;
    const { stdout, stderr, exitCode } = execResult;
    if (isLoggingEnabled()) {
        log(`EXEC exit=${exitCode} time=${duration}ms:\n\tSTDOUT: ${stdout}\n\tSTDERR: ${stderr}`);
    }
    return {
        stdout,
        stderr,
...
const isSandboxExecAvailable = fs
    .access(PATH_TO_SEATBELT_EXECUTABLE, fs.constants.X_OK)
    .then(() => true, (err) => {
    if (!["ENOENT", "ACCESS", "EPERM"].includes(err.code)) {
        log(`Unexpected error for \`stat ${PATH_TO_SEATBELT_EXECUTABLE}\`: ${err.message}`);
    }
    return false;
});
async function getSandbox(runInSandbox) {
    if (runInSandbox) {
 ...
 */
async function askUserPermission(args, applyPatchCommand, getCommandConfirmation, hooks, context) {
    const confirmation = await getCommandConfirmation(args.cmd, applyPatchCommand, context);
...
function describeManualApprovalReason(policy) {
    switch (policy) {
        case "suggest":
