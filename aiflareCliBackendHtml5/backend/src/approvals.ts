 ...
 */
export function canAutoApprove(command, workdir, policy, writableRoots, env = process.env) {
...
function canAutoApproveApplyPatch(applyPatchArg, workdir, writableRoots, policy) {
    switch (policy) {
        case "full-auto":
 ...
 */
function isWritePatchConstrainedToWritablePaths(applyPatchArg, workdir, writableRoots) {
    ...
    return (allPathsConstrainedTowritablePaths(identify_files_needed(applyPatchArg), workdir, writableRoots) &&
        allPathsConstrainedTowritablePaths(identify_files_added(applyPatchArg), workdir, writableRoots));
}
function allPathsConstrainedTowritablePaths(candidatePaths, workdir, writableRoots) {
    return candidatePaths.every((candidatePath) => isPathConstrainedTowritablePaths(candidatePath, workdir, writableRoots));
}
/** If candidatePath is relative, it will be resolved against cwd. */
function isPathConstrainedTowritablePaths(candidatePath, workdir, writableRoots) {
    const candidateAbsolutePath = resolvePathAgainstWorkdir(candidatePath, workdir);
    return writableRoots.some((writablePath) => pathContains(writablePath, candidateAbsolutePath));
}
/**
 * If not already an absolute path, resolves `candidatePath` against `workdir`
 * if specified; otherwise, against `process.cwd()`.
 */
export function resolvePathAgainstWorkdir(candidatePath, workdir) {
 ...
 */
function tryParseApplyPatch(bashArg) {
...
export function isSafeCommand(command) {
    ...
    switch (cmd0) {
        ...
        case "rg": {
            ...
            const isUnsafe = command.some((arg) => UNSAFE_OPTIONS_FOR_RIPGREP_WITHOUT_ARGS.has(arg) ||
                [...UNSAFE_OPTIONS_FOR_RIPGREP_WITH_ARGS].some((opt) => arg === opt || arg.startsWith(`${opt}=`)));
            if (isUnsafe) {
                break;
            }
            return {
        ...
        case "find": {
            ...
            if (command.some((arg) => UNSAFE_OPTIONS_FOR_FIND_COMMAND.has(arg))) {
                break;
            }
            else {
                return {
...
const UNSAFE_OPTIONS_FOR_FIND_COMMAND = new Set([
 ...
 * null.
 */
function isEntireShellExpressionSafe(parts) {
    if (parts.length === 0) {
        return null;
    }
    try {
...
function isParseEntryWithOp(entry) {
